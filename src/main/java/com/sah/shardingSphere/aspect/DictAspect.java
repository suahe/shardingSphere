...
import com.sah.shardingSphere.util.ConvertUtil;
import com.sah.shardingSphere.util.ReflectUtil;
...
public class DictAspect {
    ...
    // 定义切点Pointcut
    @Pointcut("execution(public * com.sah..*.*Controller.*(..))")
    ...
    private void parseDictText(Object result) {
        if (result instanceof CommonResponse) {
            if (((CommonResponse) result).getData() instanceof IPage) {
                ...
                for (Object record : ((IPage) ((CommonResponse) result).getData()).getRecords()) {
                ...
                ((IPage) ((CommonResponse) result).getData()).setRecords(items);
            }
        ...
        for (String dictCode : dataListMap.keySet()) {
            ...
            for (String s : dataList) {
                ...
                if (dictCode.contains(",")) {
                    String keyString = String.format(CacheConstant.SYS_CACHE_DICT_TABLE + "=::SimpleKey [%s,%s]", dictCode, data);
                    if (redisTemplate.hasKey(keyString)) {
                        try {
                            String text = ConvertUtil.getString(redisTemplate.opsForValue().get(keyString));
                    ...
                    String keyString = String.format(CacheConstant.SYS_CACHE_DICT + "::%s:%s", dictCode, data);
                    if (redisTemplate.hasKey(keyString)) {
                        try {
                            String text = ConvertUtil.getString(redisTemplate.opsForValue().get(keyString));
                            List<DictModel> list = translText.computeIfAbsent(dictCode, k -> new ArrayList<>());
            ...
            if (needTranslDataTable.size() > 0) {
                ...
                for (DictModel dict : texts) {
                    String redisKey = String.format(CacheConstant.SYS_CACHE_DICT_TABLE + "::SimpleKey [%s,%s]", dictCode, dict.getValue());
                    try {
        ...
        if (needTranslData.size() > 0) {
            ...
            for (String dictCode : manyDict.keySet()) {
                ...
                for (DictModel dict : newList) {
                    String redisKey = String.format(CacheConstant.SYS_CACHE_DICT + "::%s:%s", dictCode, dict.getValue());
                    try {
                        redisTemplate.opsForValue().set(redisKey, dict.getText());
                    } catch (Exception e) {
                        log.warn(e.getMessage(), e);
    ...
    private String translateDictValue(String code, String text, String table, String key) {
        ...
        for (String k : keys) {
            ...
            if (!StringUtils.isEmpty(table)) {
                ...
                String keyString = String.format(CacheConstant.SYS_CACHE_DICT_TABLE + "::SimpleKey [%s,%s,%s,%s]", table, text, code, k.trim());
                if (redisTemplate.hasKey(keyString)) {
                    try {
                        tmpValue = ConvertUtil.getString(redisTemplate.opsForValue().get(keyString));
                    } catch (Exception e) {
                        log.warn(e.getMessage());
                    }
                } else {
                    tmpValue = sysDictService.translateDictFromTable(table, text, code, k.trim());
                }
            } else {
                String keyString = String.format(CacheConstant.SYS_CACHE_DICT + "::%s:%s", code, k.trim());
                if (redisTemplate.hasKey(keyString)) {
                    try {
                        tmpValue = ConvertUtil.getString(redisTemplate.opsForValue().get(keyString));
                    } catch (Exception e) {
                    ...
                    tmpValue = sysDictService.translateDict(code, k.trim());
                }
            }
